let registerRender = (function ($) {
        let $userName = $('#userName'),
            ...
            $spanPassCondirm = $('#spanPassConfirm'),
            $userBio = $('#userBio'),
            $spanBio = $('#spanBio'),
            $man = $('#man'),
            $woman = $('#woman'),
        ...
        // isFlag = true;//=>记录所有表单是否都验证通过(true:都验证通过)
        ...
        let checkPhone = () => {
            ...
            if (!reg.test(value)) {
                $spanPhone.html('请输入正确的手机号！(11位数字)').addClass('error');
                return false;
            }
            //=>是否重复验证
            let code = 0;
        ...
        $plan.add(result => {
            ...
            Dialog.show('很遗憾，注册失败了，请刷新后再试试把！', {
                callback: () => {
