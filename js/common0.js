...
~function () {
    ...
    pro.myQueryURLParameter = function myQueryURLParameter() {
        ...
        this.replace(reg, (...arg) => {
            ...
            /*
            * res:本次大正则匹配的结果
            * key：本次第一个分组捕获的内容（未来作为对象的属性名）
            * value：本次第二个分组捕获的内容（未来作为对象的属性值）
            * */
            if (res.indexOf('#') > -1) {
                //=>这个是hash的处理
                obj['HASH'] = key;
    ...
    // console.log('?name=xxx&age=xxx&lx=&sex#aa'.myQueryURLParameter());
}();
...
~function () {
    class Dialog {
        ...
        constructor(con = '', {isTap = true, isAuto = 2000, callBack = new Function()} = {}) {
            //=>把后期需要使用的参数值挂载到实例上
            this.con = con;
        ...
        createMark() {
            ...
            sectionBox.innerHTML = `<div class="box"><h3>系统提示</h3><div class="content">${this.con}</div></div>`;
...
Dialog.show('提示的内容', {
    ...
    isAuto: 2000,//=>是否支持自动消失，不支持传递0，支持想让多久自动消失就传递多长时间（毫秒），默认是2000ms
    callBack: function () {
        //=>回调函数：当提示消失的时候处理什么事情
    }
});*/
...
~function () {
    ...
    $navBox.tap(function (e) {
        ...
        if (text === '登录' || text === '注册') {
            //->获取当前页面的URL地址，跳转到具体某一个页面（登录或者注册），把地址当作参数值传递过去，登录或者注册成功，返回上一个页面即可
            //=>验证当前页面地址中是否已经包含History，如果包含了，我们直接把history的值带到下一个页面即可
            let history = location.href,
                flag = text === '登录' ? 'login' : 'register';
            let isExit = history.myQueryURLParameter()['history'];
